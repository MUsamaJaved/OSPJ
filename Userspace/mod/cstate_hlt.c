...
int init_module(void)
{
	...
	printk(KERN_INFO "\n\n#################################\n");
	printk(KERN_INFO "#### Calling hlt a thousand times now#\n");

	//cpuid2(0x80000000, &a, &d);
	int i=0;
	for(i=0; i<1000; i++) {
		asm volatile ("hlt"); // hlt, hope that server will still be available
	}
	printk(KERN_INFO "#### directly after hlt-loop#\n");
	//asm("aaa"); // hlt, hope that server will still be available
	//asm("mov reg_ecx, %%ecx");
